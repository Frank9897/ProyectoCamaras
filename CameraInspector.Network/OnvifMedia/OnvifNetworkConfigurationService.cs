...
/// <summary>
/// Implementa las operaciones de escritura de red definidas por ONVIF.
/// Se mantiene separada del servicio de lectura para dejar clara la frontera READ/WRITE.
/// </summary>
public sealed class OnvifNetworkConfigurationService : IOnvifNetworkConfigurationService
{
    ...
    {
        // interfaceToken identifica la interfaz física que la cámara expone mediante ONVIF.
        if (string.IsNullOrWhiteSpace(interfaceToken))
            return Failure("La cámara no proporcionó un token de interfaz de red válido.");

        if (!useDhcp)
        {
            // Validamos la IP antes de generar XML para evitar enviar una configuración inválida.
            ...
            // PrefixLength representa la máscara en formato CIDR, por ejemplo 24 = 255.255.255.0.
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // gatewayAddress vacío limpia el gateway IPv4 siguiendo la semántica de lista opcional de ONVIF.
        ...
        var response = await OnvifSoapClient.PostAsync(endpoint, body, security, cancellationToken);
        if (response is null)
            return Failure("La cámara rechazó la modificación del gateway o no respondió.");
    ...
    private static string? ResolveDeviceServiceEndpoint(DiscoveredDevice device)
    {
        ...
        return string.IsNullOrWhiteSpace(device.IpAddress)
            ? null
            : $"http://{device.IpAddress}/onvif/device_service";
    }

    private static OnvifNetworkChangeResult Failure(string message) =>
        new()
        {
            Succeeded = false,
            Message = message
