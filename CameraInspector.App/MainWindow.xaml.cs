...
public partial class MainWindow : Window
{
    ...
    private static void SaveUtf8Csv(string filePath, string csv, string successMessage) { try { File.WriteAllText(filePath, csv, new UTF8Encoding(true)); ThemedMessageBox.Show($"{successMessage}\n\n{filePath}", "Camera Inspector — Exportación", MessageBoxButton.OK, MessageBoxImage.Information); } catch (Exception ex) { ThemedMessageBox.Show($"No se pudo guardar el CSV:\n\n{ex.Message}", "Camera Inspector — Exportación", MessageBoxButton.OK, MessageBoxImage.Error); } }
    ...
    private static void ShowInformation(string message, string title) => ThemedMessageBox.Show(message, $"Camera Inspector — {title}", MessageBoxButton.OK, MessageBoxImage.Information);
